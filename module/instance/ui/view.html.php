...
foreach($dbList as $db)
{
    ...
    $dbListWg[] = h::tr(h::td($db->db_name), h::td($db->db_type), h::td
    (
        $db->ready ? $lang->instance->dbReady : $lang->instance->dbWaiting,
        setClass('text-' . ($db->ready ? 'success' : 'danger'))
    ), h::td
    (
        btn($lang->instance->management, setClass('btn text-primary ghost db-management ' .  $disabledClass), setData('dbname', $db->name), setData('dbtype', $db->db_type), setData('id',   $instance->id))
    ));
}
...
(
    ...
    (
        ...
        detailBody(sectionList(
            /* 应用名称信息图标区块 */
            section(set::title(''), div
            (
                setClass('flex justify-between'),
                div(setClass('flex basis-full'), $type === 'store' ? img(set::src($instance->logo), setStyle(array('width' => '50px', 'height' => '50px'))) : null, div(setClass(($type === 'store' ? 'ml-3' : '') . ' flex col gap-y-1 basis-full'), div
                (
                ...
                (
                    ...
                    span(setClass('text-gray'), sprintf($lang->instance->memTotal, helper::formatKB($instanceMetric->memory->limit))),
                    div
                    (
                        setClass('progress rounded-lg'),
            ...
            (
                ...
                (
                    setStyle('min-width', '700px'),
                    setClass('table w-auto max-w-full bordered mt-4'),
                    h::tr(h::th($lang->instance->dbName), h::th($lang->instance->dbType, setStyle('width', '100px')), h::th($lang->instance->status, setStyle('width', '100px')), h::th($lang->instance->action, setStyle('width', '100px'))),
                    $dbListWg
                )
            )
        ), floatToolbar
