...
<div id='rowTip' class='hide'>
  <div class='row-tip'>
    <div class='on-expand tip'><span><?php echo $lang->repo->expand ?> </span><i class='icon-chevron-down'></i></div>
    <div class='on-collapse tip'><span><?php echo $lang->repo->collapse ?> </span><i class='icon-chevron-up'></i></div>
  </div>
</div>
<script>
  ...
  function changeProduct(select) {
    ...
    $(select).closest('.reviewFormContainer').find('select[name=execution]').parent().load(link, '', function() {
      $('#execution').chosen();
    });
    moduleLink = createLink('tree', 'ajaxGetOptionMenu', 'productID=' + productID + '&viewtype=bug&branch=0&rootModuleID=0&returnType=html');
    $(select).closest('.reviewFormContainer').find('select[name=module]').parent().load(moduleLink, '', function() {
      $('#module').chosen();
    });
  }
  ...
  function changeExecution(select) {
    ...
    $('td.taskModuleBox').load(moduleLink, '', function() {
      $('.taskModuleBox #module').attr('id', 'taskModule').attr('name', 'taskModule').chosen();
    });

    assignLink = createLink('execution', 'ajaxGetMembers', 'executionID=' + executionID);
    $('.taskAssignedToBox').load(assignLink, '', function() {
      $('.taskAssignedToBox #assignedTo').attr('id', 'taskAssignedTo').attr('name', 'taskAssignedTo').chosen();
    });
  }
