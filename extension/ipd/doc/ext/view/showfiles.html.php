...
<div id="mainMenu" class="clearfix">
  ...
  <div class="btn-toolbar pull-right">
    <?php $class = $viewType == 'card' ? 'iframe' : 'export';?>
    <?php if(common::hasPriv('doc', 'exportFiles')) echo html::a($this->createLink('doc', 'exportFiles', "objectID=$objectID&objectType=$type", "html", true), "<i class='icon-export muted'> </i>" . $lang->export, '', "class='btn btn-link $class' id='exportFiles'");?>
    <div class='btn-group'>
...
<div class="main-row fade <?php if(!empty($libTree)) echo 'flex';?>" id="mainContent">
  <div id='sideBar' class="panel side side-col col overflow-auto" data-min-width="150">
    <?php include $this->app->getModuleRoot() . 'doc/view/lefttree.html.php';?>
  </div>
  ...
  <div class="main-col flex-full overflow-visible flex-auto" data-min-width="500">
    ...
    <div class='main-table'>
      <table class="table has-sort-head" data-ride="table" id='filesTable'>
        <thead>
          <tr>
            ...
            <th class="c-id">
              <?php if(common::hasPriv('doc', 'exportFiles')) echo "<div class='checkbox-primary check-all' title='{$this->lang->selectAll}'><label></label></div>";?>
        ...
        <tbody>
          ...
          <?php if(empty($file->pathname)) continue;?>
            <tr>
              ...
              ?>
              <td><?php echo (common::hasPriv('doc', 'exportFiles') ? html::checkbox('fileIdList', array($file->id => '')) : '') . sprintf('%03d', $file->id);?></td>
        ...
        <div class="row row-grid files-grid" data-size="300">
          ...
          <div class='col'>
            <div class='lib-file'>
              ...
              ?>
              <div class='file'>
                <?php if(common::hasPriv('doc', 'exportFiles')) echo html::checkbox('fileIdList', array($file->id => ''), '', "class='fileCheckbox'");?>
...
$(function()
{
    ...
    {
        var checkedItem = [];
        var $checked    = $("input[type='checkbox'][name^='fileIdList']");
        $checked.click(function()
        {
            ...
            {
                checkedItem.splice($.inArray($(this).val(), checkedItem), 1);
            }
            $.cookie('checkedItem', checkedItem.join(','), {expires: config.cookieLife, path: config.webRoot});
        });

        $('.file').hover(function()
        {
            $(this).find('.checkbox-primary').show();
            $(this).find('.checkbox-primary').css('display', 'block');
        }, function()
        {
            if($(this).find('.checkbox-primary').find('input').prop('checked')) return;

            $(this).find('.checkbox-primary').hide();
            $(this).find('.checkbox-primary').css('display', 'none');
        })   
    }
})
</script>
